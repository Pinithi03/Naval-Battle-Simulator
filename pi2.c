...
int main() {
    ...
    fprintf(Battleship, "   -- You Survived -- \n\n");
    ...
    for ( i = 0; i < 5; i++) {
		...
		printf("-- Simulation Statistic --\n\n");
            ...
            printf("Invalid choice.Your choice must be between 1 and 4::::..\n\n");
        }
    ...
    if (num_escort_ships < 1 || num_escort_ships > 99) {
        printf("Invalid number of escort ships.Enter a number between 1 and 99.\n");
        fclose(Main);
        fclose(Escortship);
        return 1;
    }
    ...
    // Generate the movements of battleship
    for (m = 0; m < k; m++) {
 ...
 // Calculate and print maximum and minimum ranges for each escort ship
    ...
    fprintf(Main,"\nDistances between E_Ships and B_Ship:\n");
    for ( i = 0; i < num_escort_ships; i++) {
