...
int main() {
    ...
    // Initialize canvas
    char canvas[D][D][3];  // 3 chars for ship notation + null terminator
    int i, j;
    for ( i = 0; i < D; i++) {
        for (j = 0; j < D; j++) {
            canvas[i][j][0] = '\0';
    ...
    for ( i = 0; i < 4; i++) {
        ...
        strcpy(bships_array[i].type_name, bship_names[i]);
        strcpy(bships_array[i].gun_name, bgun_names[i]);
        bships_array[i].impact = 0;
        bships_array[i].angle_range = 0;
        bships_array[i].max_angle = rand() % 90;
    ...
    for (m = 0; m < k; m++) {
        ...
        do {
            x_battleship = rand() % D;
            y_battleship = rand() % D;
        } while (canvas[x_battleship][y_battleship][0] != '\0');
        ...
        printf("\nEscort Ship Projectile Ranges (Move %d):\n", m + 1);
        for ( i = 0; i < 5; i++) {
        ...
        fprintf(MainC, "\nDistances between Escort Ships and BattleShip (Move %d):\n", m + 1);
        for ( i = 0; i < num_escort_ships; i++) {
        ...
        for (i = 0; i < num_escort_ships; i++) {
            float eshell_impact = eimpacts[i % 5];
    ...
    fprintf(Final, "Total time taken for the battle: %.2f seconds.\n", total_time);
